...
if (argc == 3 && strcmp(argv[1], "-c") == 0){
    ...
    if (is_command_with_batch(argv[2])) {
        launch_program_with_batch(argv[2]);
    } else {
        // Not a batch command, process as single command
        char line[MAX_LINE];
        strcpy(line, argv[2]);
